...
export default async function BillingPage({searchParams}:{searchParams:Promise<{filter?:string}>}) {
  ...
  const filters=[["open","未請求・未入金"],["overdue","期限超過"],["planned","未請求"],["invoiced","請求済"],["paid","入金済"],["all","すべて"]];
  return <>
    <div className="page-head"><div><h1>請求・入金</h1><p className="muted">案件の売上を、未請求・請求済・入金済まで追跡します。</p></div></div>
    <div className="grid-4 billing-kpis">
      <MetricCard label="未請求" value={yen(snapshot.unbilledAmount)} note={`請求対象未登録 ${yen(snapshot.unbilledReadyAmount)} / 予定登録 ${yen(snapshot.plannedAmount)}`}/>
      ...
      <MetricCard label="入金済" value={yen(snapshot.paidAmount)} note={`請求累計 ${yen(snapshot.issuedAmount)}`}/>
    </div>
    {snapshot.unbilledProjects.length > 0 && (filter === "open" || filter === "planned") && <section className="card billing-ready-card">
      ...
      <div className="list">{snapshot.unbilledProjects.map((p)=><div className="list-row" key={p.projectId}><div className="grow"><div className="list-title">{p.projectName}</div><div className="small muted">{p.companyName}{p.units != null ? ` ・ ${p.units}${p.unitLabel ?? "回"} が請求対象` : ""}</div></div><strong className="money-value">{yen(p.amount)}</strong><Link className="button primary" href={`/projects/${p.projectId}/billing/new`}>請求予定を作る</Link></div>)}</div>
    </section>}

    <div className="filter-chips">{filters.map(([v,l])=><Link className={`filter-chip ${filter===v?"active":""}`} href={`/billing?filter=${v}`} key={v}>{l}</Link>)}</div>
    <section className="card">
      <div className="card-head"><h2>請求一覧</h2><span className="badge">{rows.length}件</span></div>
      <div className="table-wrap"><table className="billing-table"><thead><tr><th>案件</th><th>請求名</th><th>状態</th><th>金額</th><th>支払期限</th><th>入金日</th><th>操作</th></tr></thead><tbody>
        {rows.map(inv=><tr key={inv.id} className={inv.overdue?"billing-overdue-row":inv.status==="cancelled"?"billing-cancelled-row":""}>
          ...
          <td>{inv.title}{inv.referenceNo&&<div className="small muted">{inv.referenceNo}</div>}</td>
          ...
          <td className="money-value"><strong>{yen(inv.amount)}</strong></td><td>{inv.dueDate??"—"}</td><td>{inv.paidDate??"—"}</td>
