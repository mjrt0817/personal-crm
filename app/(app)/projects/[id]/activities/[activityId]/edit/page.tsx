...
export default async function ActivityEditPage({ params }: { params: Promise<{ id: string; activityId: string }> }) {
  ...
  return <>
    ...
    <form action={updateActivity} className="form-card form-padded">
      <input type="hidden" name="id" value={activity.id}/><input type="hidden" name="project_id" value={id}/><input type="hidden" name="old_project_id" value={id}/><input type="hidden" name="company_id" value={project.companyId}/><input type="hidden" name="return_to" value={`/projects/${id}?tab=activities`}/>
      ...
      <div className="form-actions"><a className="button" href={`/projects/${id}?tab=activities`}>キャンセル</a><SubmitButton pendingLabel="保存中…">変更を保存</SubmitButton></div>
    </form>
  </>;
}
