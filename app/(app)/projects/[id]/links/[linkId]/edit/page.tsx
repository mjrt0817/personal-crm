...
export default async function LinkEditPage({ params }: { params: Promise<{ id: string; linkId: string }> }) {
  ...
  return <>
    ...
    <form action={updateProjectLink} className="form-card">
      <input type="hidden" name="id" value={link.id}/><input type="hidden" name="project_id" value={id}/><input type="hidden" name="return_to" value={`/projects/${id}#links`}/>
      ...
      <div className="form-actions"><a className="button" href={`/projects/${id}#links`}>キャンセル</a><SubmitButton pendingLabel="保存中…">変更を保存</SubmitButton></div>
    </form>
  </>;
}
