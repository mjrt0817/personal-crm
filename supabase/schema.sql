...
create table if not exists public.projects (
  ...
  pricing_model text not null default 'fixed' check (pricing_model in ('fixed','unit')),
  unit_label text not null default '回',
  unit_price numeric(12,0) check (unit_price is null or unit_price >= 0),
  planned_units numeric(8,2) check (planned_units is null or planned_units >= 0),
  completed_units numeric(8,2) check (completed_units is null or completed_units >= 0),
...
create index if not exists project_invoices_user_status_due_idx on public.project_invoices(user_id, status, due_date);
create index if not exists project_invoices_project_created_idx on public.project_invoices(project_id, created_at desc);
create index if not exists project_invoices_company_idx on public.project_invoices(company_id);
...
create trigger set_project_invoices_updated_at before update on public.project_invoices for each row execute function public.set_updated_at();
